...
let project_one ~max_depth ~max_nodes ~classify ~bases provenance selection =
  ...
  let visit () (n : Provenance.node) =
    ...
    (match n.kind with
     ...
     | Operation name ->
         ...
         List.iter(fun(role,_)->let i=role_index role in roles.(i)<-roles.(i)+1;
         ...
         (match n.step_index,n.origin with
          |Some step,Some origin ->
              ...
              List.iter(fun(role,_)->increment_role p role;
    ...
    ()
  in
  let ()=Provenance.fold_reachable provenance ~roots:[root] ~init:() ~f:visit in
  ...
  let producer_location_count =
    ...
    List.iter (fun p -> Hashtbl.replace locations (producer_key p.image p.image_offset p.runtime_pc) ()) producer_rows;
...
let to_json_string report =
  ...
  add "],\"selections\":[";
  List.iteri(fun si p->comma out si;add "{\"sink\":{\"file\":";add(file_json p.sink.file);Printf.bprintf out ",\"offset\":%d,\"generation\":%s},\"value\":%s,\"write_step\":%s,\"root\":" p.sink.offset (opt_int p.sink.generation) (opt_int p.sink_value)(opt_int p.write_step);
