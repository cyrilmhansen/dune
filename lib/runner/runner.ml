...
let run_loaded ~max_steps ~on_step ~output memory loaded =
  ...
  let rec run steps =
    if I8080.State.pc state = 0x0005 then
      ...
      | Ok Cpm.Bdos.Terminate ->
          Ok { termination = Bdos_function (I8080.State.c state); steps }
...
let run_with_loader ~max_steps ~on_step ~output load =
  if max_steps <= 0 then Error (Invalid_step_limit max_steps)
    ...
    | Ok loaded -> run_loaded ~max_steps ~on_step ~output memory loaded

let run_bytes ?(max_steps = default_max_steps) ?(on_step = fun _ -> ()) ~output bytes =
  run_with_loader ~max_steps ~on_step ~output (fun memory ->
      Cpm.Loader.load_bytes memory bytes)

let run_file ?(max_steps = default_max_steps) ?(on_step = fun _ -> ()) ~output ~path () =
  run_with_loader ~max_steps ~on_step ~output (fun memory ->
