...
const viewerTables = {};
for (const [i,id] of ["#producers","#sources","#operations","#source-ranges","#output-table"].entries()) viewerTables[id]=initRows[i];

const roleColor={value:"#2878b5",address:"#d18a16",flag:"#bc3b82",control:"#555"};
let graph=null, current=null, currentProducer=null;
...
function graphData(p){
  ...
  combos.push({id:"group:root",data:{label:"selected sink"}},{id:"group:operations",parentId:"group:root",data:{label:"operations"}},{id:"group:sources",parentId:"group:root",data:{label:"sources"}});
  add("root",{data:{label:`root #${p.root}`,kind:"root"}});
  for(const n of p.preview.nodes){
  ...
  for(const e of p.preview.edges)edges.push({id:`edge:${e.from}:${e.to}:${e.role}:${edges.length}`,source:`node:${e.from}`,target:`node:${e.to}`,data:{role:e.role,label:e.role},style:{stroke:roleColor[e.role],lineWidth:2}});
  if(typeof p.root==="number")edges.push({id:`rootedge:${p.root}`,source:"root",target:`node:${p.root}`,data:{role:"slice root"},style:{stroke:"#a8b2bc"}});
  return {nodes,edges,combos};
}
function producerOverlay(p){
...
function renderHeatmaps(p){
  const root=document.querySelector("#heatmaps");root.replaceChildren();const mode=document.querySelector("#mode").value,overlay=producerOverlay(p);
  ...
  for(const im of report.execution.images){const wrap=document.createElement("div"),label=document.createElement("strong"),canvas=document.createElement("canvas"),tip=document.createElement("div");wrap.className="image-map";label.textContent=`${im.display_name} · virtual ${im.virtual_base.toString(16)}–${(im.virtual_base+im.span-1).toString(16)} · ${im.span} bytes`;tip.className="tip";canvas.className="heatmap";canvas.width=Math.max(1,im.span);canvas.height=28;const ctx=canvas.getContext("2d"),max=Math.max(1,...im.bytes.map(b=>b[1]));
    for(let off=0;off<im.span;off++){const b=im.bytes[off],v=overlay.get(im.virtual_base+off);let intensity=0,color="#e4e8ed";
      ...
      else {let count=mode==="participation"?(v?1:0):v?.[heatKey]??0;intensity=count?Math.log1p(count)/Math.log1p(overlayMaximum):0;color=count?`rgba(196,68,53,${0.1+0.9*intensity})`:"#e4e8ed";}
...
function highlightProducer(id){currentProducer=current?.producers.find(p=>p.id===id)??null;document.querySelector("#producer-highlight").textContent=`Selected producer location: ${id}`;if(graph){const nodeId=current?.preview.nodes.find(n=>n.origin&&producerLocationId(n.origin)===id);if(nodeId)graph.setElementState(`node:${nodeId.id}`,"selected");}if(current)renderHeatmaps(current);}
async function setTableData(p){const rows=groupedRows(p);for(const [id,data] of [["#producers",rows.producers],["#sources",rows.sources],["#operations",rows.operations],["#source-ranges",rows.sourceRanges],["#output-table",report.output_bytes]]){const old=viewerTables[id];const table=await worker.table(data.length?data:[{empty:"no rows"}]);await old.viewer.load(table);const group_by=id==="#producers"?["image","operation_kind"]:id==="#sources"?["classification","identity"]:id==="#source-ranges"?["classification","identity"]:id==="#operations"?["kind"]:[];const sort=id==="#producers"?[["producer_steps","desc"]]:id==="#operations"?[["node_count","desc"]]:id==="#source-ranges"?[["first","asc"]]:[["offset","asc"]];await old.viewer.restore({plugin:"Datagrid",group_by,sort});old.table.delete?.();viewerTables[id]={...old,table,rows:data.length};window.__tableGroups??={};window.__tableGroups[id]=group_by;}
  window.__perspectiveRows=Object.fromEntries(Object.entries(viewerTables).map(([k,v])=>[k,v.rows]));window.__producerRows=rows.producers;window.__producerIds=rows.producers.map(x=>x.producer_id);}
async function renderGraph(p){const data=graphData(p);if(graph){graph.destroy();graph=null;}graph=new Graph({container:"graph",width:document.querySelector("#graph").clientWidth,height:540,data,layout:{type:"dagre",rankdir:"LR",nodesep:18,ranksep:40},behaviors:["drag-canvas","zoom-canvas",{type:"collapse-expand",trigger:"click",animation:false}],node:{style:{labelText:d=>d.data?.label??d.id,size:18}},edge:{style:{stroke:d=>roleColor[d.data?.role]??"#a8b2bc",endArrow:true,labelText:d=>d.data?.role??""}},combo:{type:"rect",style:{labelText:d=>d.data?.label??d.id,collapsed:false}}});await graph.render();graph.on("node:click",event=>{const id=event.target?.id??event.data?.id;const d=data.nodes.find(n=>n.id===id)?.data;if(d?.producer_id)highlightProducer(d.producer_id)});graph.on("combo:click",event=>{const id=event.target?.id??event.data?.id;const d=data.combos.find(c=>c.id===id)?.data;if(d?.producer_id)highlightProducer(d.producer_id)});window.__emitG6Producer=id=>{const n=data.nodes.find(x=>x.data?.producer_id===id);if(n)graph.emit("node:click",{target:{id:n.id}});else{const c=data.combos.find(x=>x.data?.producer_id===id);if(c)graph.emit("combo:click",{target:{id:c.id}})}};document.querySelector("#graph-meta").textContent=`G6 bounded graph: ${data.nodes.length} nodes, ${data.edges.length} edges, ${data.combos.length} combos (preview max ${p.preview.max_nodes} nodes, depth ${p.preview.max_depth}, omitted frontier ${p.preview.omitted_frontier_count}).`;
  window.__g6Counts={nodes:data.nodes.length,edges:data.edges.length,combos:data.combos.length};}
async function renderProjection(p){current=p;window.__explorerReady=false;safeText(document.querySelector("#sink-meta"),`${p.sink.file.name} +${p.sink.offset.toString(16).toUpperCase().padStart(4,"0")}h = ${p.value?.toString(16).toUpperCase().padStart(2,"0")}h · write step ${p.write_step} · root ${p.root} · ${p.full_node_count} exact slice nodes · ${p.source_leaf_count} source leaves`);
  ...
  roleSummary(p);renderHeatmaps(p);await Promise.all([setTableData(p),renderGraph(p)]);
  const preview=document.querySelector("#preview");preview.replaceChildren();document.querySelector("#preview-meta").textContent=`Bounded exact neighborhood: ${p.preview.visible_nodes} visible nodes / ${p.preview.max_nodes} maximum; depth ${p.preview.max_depth}; omitted frontier ${p.preview.omitted_frontier_count}. Not the complete DAG.`;
  for(const n of p.preview.nodes){const div=document.createElement("div");div.className="preview-node";div.textContent=`#${n.id} ${n.label} value=${n.value.toString(16)} width=${n.width} step=${n.step??"-"} origin=${n.origin?`${n.origin.image.name}+${n.origin.offset.toString(16)} @${n.origin.runtime_pc.toString(16)}`:"-"}`;preview.append(div)}
  for(const button of outputGrid.children)button.classList.toggle("selected",Number(button.dataset.offset)===p.sink.offset);
  window.__selectedSink=p.sink.offset;window.__explorerReady=true;window.__selectProducer=id=>highlightProducer(id);window.__heatMode=document.querySelector("#mode").value;window.__roleOverlayCounts=Object.fromEntries(["value","address","flag","control"].map(k=>[k,p.producers.reduce((n,x)=>n+x[`${k}_edges`],0)]));}
async function clearProjection(){current=null;currentProducer=null;document.querySelector("#heatmaps").replaceChildren();document.querySelector("#source-data").replaceChildren();document.querySelector("#source-summary").replaceChildren();document.querySelector("#roles").replaceChildren();document.querySelector("#preview").replaceChildren();document.querySelector("#preview-meta").textContent="No detailed projection embedded for this output byte.";document.querySelector("#graph-meta").textContent="No detailed projection embedded.";document.querySelector("#producer-highlight").textContent="";if(graph){graph.destroy();graph=null;}for(const id of ["#producers","#sources","#operations","#source-ranges"]){const old=viewerTables[id],table=await worker.table([{status:"No projection embedded"}]);await old.viewer.load(table);await old.viewer.restore({plugin:"Datagrid"});old.table.delete?.();viewerTables[id]={...old,table,rows:0};}window.__perspectiveRows=Object.fromEntries(Object.entries(viewerTables).map(([k,v])=>[k,v.rows]));}
...
document.querySelector("#sink").addEventListener("change",e=>void selectOffset(Number(e.target.value)));document.querySelector("#mode").addEventListener("change",()=>current&&renderHeatmaps(current));
...
window.__reportLoaded={images:report.execution.images.length,outputBytes:report.output_bytes.length,selectedProjections:report.selections.length};
